...
function Messages_sended() {

    const [messageList , SetMessageList] = useState([]);

    useEffect(()=> {
        axios.get('/message/sended/'+sessionStorage.getItem("id")).then((res)=>{
      ...
      return (
        <div>
          <div class="receive-message-box">보낸 쪽지함</div>
          <div className="groupBox">      
            {
              messageList.map(function(a,i){
                return(     
                  <div>
                    <Link to={'/message/detail/sended/'+a.message_id} className="message-content">       
...
export default Messages_sended;